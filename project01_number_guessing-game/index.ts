...
async function askQuestion(ComputerChoice:number){
  ...
  .then((answers) => {
    ...
    let userchoice:Number = answers.number
...
function Computernumber (){
    let C_choice: number = Math.round(Math.random()*10);
    return C_choice
}


async function startAgain () {
    let Comp_choice: number = Computernumber();
    //console.log("Comp_choice = " + Comp_choice);
    await Welcome();
    let Score = 0;
    let Attempts = 0;
   do{
    let a:boolean = await askQuestion(Comp_choice);
