...
async function Welcome(credentials: {userid:number; userpin:number;}) {
...
function UserID() {
    let ID_choice:number = Math.round(Math.random() * 10000);
    return ID_choice;
}
function UserPin() {
    let PIN_choice:number = Math.round(Math.random() * 1000000);
    return PIN_choice;
}

async function authenticate(arr:{userid:number; userpin:number;}) {
...
async function validate (){
    ...
    .then(async (answers) => {
      ...
      if (answers.operator == "Login") {
        let flag:boolean = await authenticate(details);
        if(flag === true){
            console.clear();
            //ATM Options
            let Acc_Balance:number = 0;
            do{
            console.clear();
...
async function Options(Account_Balance:number){
    await inquirer
  .prompt([
    /* Pass your questions in here */
    {
